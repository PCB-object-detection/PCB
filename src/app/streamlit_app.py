...
from collections import deque, Counter  # 추가
import cv2
import time
import os
import pandas as pd
...
# [설정] 페이지 기본 설정
# ==========================================
BASE_DIR = Path(__file__).resolve().parents[2] # 동적 path
MODEL_DIR = BASE_DIR / "weights" # original folder name: models
...
if 'inference_times' not in st.session_state:
    st.session_state.inference_times = deque(maxlen=30)  # 추가
...
def summarize_defects(results):  # 추가
    counter = Counter()
...
with st.sidebar:
    ...
    "Baseline (기본)": "best_yolov11m_imgsz1280.pt",
...
with tab_dashboard:
    ...
    with col_visual:
        ...
        if input_mode == "📷 이미지 업로드" and uploaded_file:
            ...
            st.session_state.inference_times.append(st.session_state.last_inference_time)  # 추가
            ...
            if defect_count > 0:
                ...
                # 상세 요약 메시지 추가
                defect_summary = summarize_defects(results)  # 추가
                if defect_summary:
                    detail_msg = " | ".join([f"{cls}({cnt})" for cls, cnt in defect_summary.items()])  # 추가
                    status_placeholder.error(f"상세: {detail_msg}", icon="📋")  # 추가
            else:
            ...
            if 'last_processed_file' not in st.session_state or st.session_state.last_processed_file != file_id:
                ...
                if defect_count > 0:
                    ...
                    defect_summary = summarize_defects(results)  # 추가
                    if defect_summary:
                        summary_str = " | ".join([f"{cls}({cnt})" for cls, cnt in defect_summary.items()])  # 추가
                        st.session_state.detection_log.insert(0, f"[{ts}] NG-DETAIL: {summary_str}")  # 추가
                else:
            ...
            if st.session_state.webcam_on and st.session_state.cap is not None:
                ...
                if ret:
                    ...
                    webcam_spot.markdown(
                        f'<img src="data:image/png;base64,{b64_web}" class="fixed-img">',
                        unsafe_allow_html=True
                    )

                    if len(results.boxes) > 0:
                        status_placeholder.error("🚨 결함 탐지중")
                    else:
                        status_placeholder.success("✅ 정상")
    ...
    with col_monitor:
        ...
        # 평균 FPS 카드 추가
        avg_fps = get_avg_fps()  # 추가
        st.markdown(f'<div class="metric-container"><p class="metric-value" style="color:#27ae60;">{avg_fps:.1f}</p><p class="metric-label">Avg FPS</p></div>', unsafe_allow_html=True)  # 추가
